...
export class TravelsService {
    ...
    public travelsList$ = this.travelsListSubject.asObservable(); constructor() {
    ...
    getTravelsList(): Observable<TravelListItem[]> {
        ...
        return of(this.travelsListSubject.value).pipe(
    ...
    getTravelDetails(travelId: string): Observable<TravelDetails | null> {
        ...
        const travel = this.mockTravels.find(t => t.id === travelId);
        ...
        // Construir los detalles completos del viaje
        const travelDetails: TravelDetails = {
            ...
            mapData: {
                ...
                estimatedDuration: travel.estimatedDuration,
                distance: travel.distance,
                routeStatus: this.mapRouteStatus(travel.status)
            },
            routeInfo: {
                id: travel.id,
                origin: travel.origin,
                destination: travel.destination,
                updates: this.generateRouteUpdates(travel),
                estimatedDuration: travel.estimatedDuration,
                distance: travel.distance,
                status: this.mapRouteStatus(travel.status)
            },
            driverInfo: travel.driverId ? this.mockDrivers[travel.driverId] : undefined
        };

        return of(travelDetails).pipe(
    ...
    /**
     * Simula la creación de un nuevo viaje
     */
    createTravel(travelData: Partial<Travel>): Observable<Travel> {
        this.isLoadingSubject.next(true); const newTravel: Travel = {
            id: `travel-${Date.now()}`,
            name: travelData.name || 'Nuevo Viaje',
            status: 'solicitud-servicio',
            origin: travelData.origin!,
            destination: travelData.destination!,
            createdAt: new Date(),
            scheduledAt: travelData.scheduledAt,
            estimatedDuration: travelData.estimatedDuration || 20,
            distance: travelData.distance || 10,
        ...
        this.mockTravels.push(newTravel);
        this.initializeMockData(); // Actualizar la lista reactiva

        return of(newTravel).pipe(
            delay(1000)
        );
    }
